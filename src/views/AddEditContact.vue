...
<template>
    <div class="contact__setting-page">
        <div class="container">
            <h2 class="contact__title">{{formData.id ? 'Изменить' : 'Добавить'}} контакт</h2>
            <form action="" class="contact__form" @submit.prevent="sendData">
                <AppInput label-text="Ф.И.О" 
                    placeholder="Ваше Ф.И.О" 
                    class="contact__item" required v-model="store.$state.formData.name"/>
                <AppInput label-text="Номер" 
                    placeholder="Ваш Номер"
                    type="tel" 
                    class="contact__item" required v-model="store.$state.formData.phone"/>
                <AppInput label-text="Почта" 
                    placeholder="Ваш адрес электронной почты" 
                    type="email"
                    class="contact__item" v-model="store.$state.formData.email"/>
                <AppInput label-text="Теги" 
                    placeholder="Введите какие-нибудь теги для контакта" 
                    class="contact__item" v-model="store.$state.formData.tags"/>
